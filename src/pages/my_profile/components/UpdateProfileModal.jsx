...
export const UpdateProfileModal = ({setUpdateProfileModal, userActualName, fetchAllData}) => {
    
    const { loggedUser, setSomethingChanged, somethingChanged } = useUserContext();
        ...
        <div className={style.change_password_modal_area}>
            <div className={style.change_password_modal_content}>
                <div className={style.change_password_modal_close_button}>
                    <button onClick={() => setUpdateProfileModal(false)}><ArrowBackIos sx={{color: "#004643"}} /> Close</button>
                </div>
                <h1>Update profile</h1>
                <div className={style.change_password_modal_initial}>
                    <p>Change your name or profile picture</p>
                    <input type="text" placeholder="Name" value={userNewName} onChange={handleNameChange}/>
