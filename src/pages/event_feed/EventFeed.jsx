...
export const EventFeed = () => {
    ...
    return(
        <div className={style.feed_area}>
            ...
            <h2 className={style.event_feed_title} onClick={() => console.log(eventFeed)}>Event feed: {event.title}</h2>
            ...
            <div className={style.recomendations}>
                ...
                <div className={style.popular_users}>
                    ...
                    <div className={style.popular_users_search}>
                        ...
                        <input type="text" placeholder="Search for a member" value={memberSearchTerm} onChange={handleMemberSearchTerm}/>
                    </div>
                    }
                    {event.organizerId == loggedUser && eventMembers.filter(member => member.id != loggedUser).length > 0 &&
                        <div className={style.organizer_options}>
                            <button onClick={() => setOrganizerOptions("search")} className={organizerOptions == "search" ? style.organizer_option_active : style.organizer_option_inactive}>Search member</button>
                            <button onClick={() => setOrganizerOptions("remove")} className={organizerOptions == "remove" ? style.organizer_option_active : style.organizer_option_inactive}>Remove member</button>
                        </div>
                    }
                    {eventMembers.filter(member => member.id != loggedUser).length == 0 &&
                    ...
                    {memberSearchTerm == "" &&
                        <>
                            {eventMembers.filter(member => member.id != loggedUser).map((member, index) => (
                                <UserCard key={member.id} openRemoveMemberModal={openRemoveMemberModal} activeTab={organizerOptions} userId={member.id} userName={member.name} profilePic={member.profilePicPath} usertype={member.type} followers={member.followers} following={member.following} posts={member.posts} fetchPopulars={fetchAllData} followingIds={followingIds}/>
                            ))}
                        </>
                    }
                    {memberSearchTerm != "" &&
                        <>
                            {eventMembers.filter(member => member.id != loggedUser && member.name.toLowerCase().includes(memberSearchTerm.toLowerCase())).map((member, index) => (
                                <UserCard key={member.id} openRemoveMemberModal={openRemoveMemberModal} activeTab={organizerOptions} userId={member.id} userName={member.name} profilePic={member.profilePicPath} usertype={member.type} followers={member.followers} following={member.following} posts={member.posts} fetchPopulars={fetchAllData} followingIds={followingIds}/>
                            ))}
                        </>
                    }
                    {memberSearchTerm != "" && eventMembers.filter(member => member.id != loggedUser && member.name.toLowerCase().includes(memberSearchTerm.toLowerCase())).length == 0 &&
                        <>
            ...
            {removeMemberModalIsOpen &&
                <RemoveMemberModal setRemoveMemberModalIsOpen={setRemoveMemberModalIsOpen} userId={choosedMember} eventId={id} fetchAllData={fetchAllData}/>
            }
        </div>
    )
}