...
export const RemoveMemberModal = ({ setRemoveMemberModalIsOpen, userId, eventId, fetchAllData }) => {
    ...
    const handleRemoveMember = async () => {
        try {
            await api.delete(`/subscription/cancel/${userId}/${eventId}`);
            fetchAllData();
    ...
    return (
        <div className={style.delete_post_modal}>
            <div className={style.delete_area}>
                ...
                <button className={style.confirm_delete} onClick={handleRemoveMember}>
                    Remove
                </button>
                <button className={style.cancel_delete} onClick={() => setRemoveMemberModalIsOpen(false)}>
                    Cancel
                </button>
            </div>
        </div>
